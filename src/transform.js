...
const getLogisticsInfo = logistics => {
    const { food_covered, food_weekends, accommodation_covered, accommodation_provided } = logistics
    return {
...
const getLegalInfo = legalInfo => {
    const { visa_link, visa_type, visa_duration, health_insurance_info } = legalInfo
    return {
        visaLink: visa_link,
        visaType: visa_type,
...
const getSdgInfo = sdgInfo => {
    ...
    const { id, target, parent_id, description, goal_id, goal_index, target_index } = sdg_target
    return {
        id: sdgInfo.id,
        sdgTarget: {
            id,
